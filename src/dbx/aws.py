...
def build_user_data(config: AppConfig, request: JobLaunchRequest) -> str:
    ...
    script = [
        ...
        f"CODEX_API_KEY={shlex.quote(config.codex_api_key or '')}",
        f"CODEX_AGENT_IDENTITY={shlex.quote(config.codex_agent_identity or '')}",
        ...
        ")",
        "PY",
        "  chown \"$DBX_USER\":\"$DBX_USER\" \"$STATUS_JSON\" \"$STATUS_FILE\"",
        "}",
        "on_error() {",
        "  local exit_code=$?",
        ...
        "cat >\"/usr/local/bin/dbx-codex-auth-bootstrap\" <<'DBX_CODEX_AUTH_BOOTSTRAP_EOF'",
        "#!/bin/bash",
        "set -euo pipefail",
        f"CODEX_API_KEY={shlex.quote(config.codex_api_key or '')}",
        f"CODEX_AGENT_IDENTITY={shlex.quote(config.codex_agent_identity or '')}",
        "if [ -n \"$CODEX_AGENT_IDENTITY\" ]; then",
        "  printf '%s' \"$CODEX_AGENT_IDENTITY\" | codex login --with-agent-identity >/dev/null",
        "  exit 0",
        "fi",
        ...
        "fi",
        "exit 1",
        "DBX_CODEX_AUTH_BOOTSTRAP_EOF",
        "chmod 755 /usr/local/bin/dbx-codex-auth-bootstrap",
        "cat >\"/usr/local/bin/dbx-start-runtime\" <<'DBX_START_RUNTIME_EOF'",
        "#!/bin/bash",
        "set -euo pipefail",
        f"REPO_DIR={shlex.quote(repo_dir)}",
        f"STATUS_FILE={shlex.quote(status_file)}",
        ...
        "  local state=\"$2\"",
        "  local detail=\"${3:-}\"",
        "  python3 - \"$STATUS_JSON\" \"$STATUS_FILE\" \"$phase\" \"$state\" \"$detail\" <<'PY'",
        "import json",
        "import sys",
        "from pathlib import Path",
        "status_json_path, status_file_path, phase, state, detail = sys.argv[1:]",
        "payload = json.loads(Path(status_json_path).read_text(encoding='utf-8')) if Path(status_json_path).exists() else {}",
        "payload.update({'phase': phase, 'state': state, 'detail': detail})",
        "Path(status_json_path).write_text(json.dumps(payload, indent=2) + '\\n', encoding='utf-8')",
        "Path(status_file_path).write_text(",
        "    '\\n'.join([",
        ...
        "PY",
        "}",
        "tee \"$HOME/.codex/hooks.json\" >/dev/null <<'DBX_RUNTIME_HOOKS_EOF'",
        *runtime_hooks_json.splitlines(),
        "DBX_RUNTIME_HOOKS_EOF",
        "if tmux has-session -t \"$SESSION_NAME\" >/dev/null 2>&1; then",
        "  write_runtime_status \"runtime\" \"starting\" \"waiting_for_agent_heartbeat\"",
        "  exit 0",
        "fi",
        "rm -f \"$BLOCKER_FILE\"",
        "write_runtime_status \"bootstrap\" \"running\" \"starting tmux codex session\"",
        ...
        "  write_runtime_status \"bootstrap\" \"waiting\" \"waiting for codex resume session upload\"",
        ...
        "  done",
        "  if [ ! -f \"$RESUME_SESSION_REMOTE_PATH\" ]; then",
        "    write_runtime_status \"bootstrap\" \"blocked\" \"codex resume session upload missing\"",
        "    cat >\"$BLOCKER_FILE\" <<EOF",
        ...
        "  fi",
        "fi",
        "tmux send-keys -t \"$SESSION_NAME\":0.0 /usr/local/bin/dbx-codex-watch C-m",
        "write_runtime_status \"runtime\" \"starting\" \"waiting_for_agent_heartbeat\"",
        "DBX_START_RUNTIME_EOF",
        "chmod 755 /usr/local/bin/dbx-start-runtime",
        "cat >\"/usr/local/bin/dbx-auth-recover\" <<'DBX_AUTH_RECOVER_EOF'",
        "#!/bin/bash",
        "set -euo pipefail",
        "codex logout >/dev/null 2>&1 || true",
        "codex login --device-auth",
        "/usr/local/bin/dbx-codex-preflight",
        "/usr/local/bin/dbx-start-runtime",
        "DBX_AUTH_RECOVER_EOF",
        "chmod 755 /usr/local/bin/dbx-auth-recover",
        ...
        "  local detail=\"$1\"",
        "  python3 - \"$STATUS_JSON\" \"$STATUS_FILE\" \"$detail\" <<'PY'",
        "import json",
        "import sys",
        "from pathlib import Path",
        "status_json_path, status_file_path, detail = sys.argv[1:]",
        "payload = json.loads(Path(status_json_path).read_text(encoding='utf-8')) if Path(status_json_path).exists() else {}",
        "payload.update({'phase': 'bootstrap', 'state': 'blocked', 'detail': detail})",
        "Path(status_json_path).write_text(json.dumps(payload, indent=2) + '\\n', encoding='utf-8')",
        "Path(status_file_path).write_text(",
        "    '\\n'.join([",
        "        f\"# {payload.get('job_name', 'dbx job')}\",",
        ...
        "PY",
        "}",
        "redact_preflight_tail() {",
        "  python3 - \"$PREFLIGHT_TAIL_RAW\" \"$PREFLIGHT_TAIL_REDACTED\" <<'PY'",
        "import re",
        "import sys",
        "from pathlib import Path",
        ...
        "",
        "## Recovery",
        "Run /usr/local/bin/dbx-auth-recover after signing in.",
        "",
        "## preflight.log tail",
        "EOF",
        ...
        "    raise SystemExit(0)",
        "if text.count('DBX_PREFLIGHT_OK') >= 2:",
        "    print('ok')",
        "    raise SystemExit(0)",
        "print('pending')",
        "PY",
        ...
        *preflight_hooks_json.splitlines(),
        ...
        "write_codex_auth_blocker() {",
        "  local detail=\"$1\"",
        "  local evidence_file=\"$2\"",
        "  local title='# Codex authentication required'",
        "  local summary='Codex is not signed in for the ubuntu user on this VM.'",
        "  local evidence_heading='## codex login status'",
        "  case \"$detail\" in",
        "    codex_auth_bootstrap_failed)",
        "      title='# Codex authentication bootstrap failed'",
        "      summary='dbx could not bootstrap Codex auth automatically from the configured credential source.'",
        "      ;;",
        "    codex_auth_failed)",
        "      title='# Codex authentication failed'",
        "      summary='dbx detected a Codex authentication failure during bootstrap.'",
        "      evidence_heading='## auth failure detail'",
        "      ;;",
        "  esac",
        "  cat >\"$BLOCKER_FILE\" <<EOF",
        "$title",
        ...
        "",
        "## Recovery",
        "1. SSH to the VM: ssh $DBX_USER@$SESSION_NAME",
        "2. Run /usr/local/bin/dbx-auth-recover after signing in.",
        "3. Re-run dbx status or dbx monitor from your local machine.",
        "",
        "$evidence_heading",
        "EOF",
        "  if [ -n \"$evidence_file\" ] && [ -f \"$evidence_file\" ]; then",
        "    cat \"$evidence_file\" >> \"$BLOCKER_FILE\"",
        "  fi",
        "}",
        "if [ -n \"$CODEX_AGENT_IDENTITY\" ] || [ -n \"$CODEX_API_KEY\" ]; then",
        "  write_status \"bootstrap\" \"running\" \"bootstrapping codex auth\"",
        "  set +e",
        "  sudo -u \"$DBX_USER\" -H /usr/local/bin/dbx-codex-auth-bootstrap",
        "  AUTH_BOOTSTRAP_EXIT=$?",
        "  set -e",
        "  if [ \"$AUTH_BOOTSTRAP_EXIT\" -ne 0 ]; then",
        "    write_status \"bootstrap\" \"blocked\" \"codex_auth_bootstrap_failed\"",
        "    : > \"$AUTH_STATUS_RAW\"",
        "    printf '%s\\n' 'Configured Codex auth bootstrap failed.' >> \"$AUTH_STATUS_RAW\"",
        "    redact_text_file \"$AUTH_STATUS_RAW\" \"$AUTH_STATUS_REDACTED\"",
        "    write_codex_auth_blocker codex_auth_bootstrap_failed \"$AUTH_STATUS_REDACTED\"",
        ...
        "  redact_text_file \"$AUTH_STATUS_RAW\" \"$AUTH_STATUS_REDACTED\"",
        "  write_status \"bootstrap\" \"blocked\" \"codex_login_required\"",
        "  write_codex_auth_blocker codex_login_required \"$AUTH_STATUS_REDACTED\"",
        "  rm -f \"$AUTH_STATUS_RAW\" \"$AUTH_STATUS_REDACTED\"",
        "  exit 0",
        "fi",
        "rm -f \"$AUTH_STATUS_RAW\" \"$AUTH_STATUS_REDACTED\"",
        ...
        "  exit 0",
        "fi",
        "sudo -u \"$DBX_USER\" -H /usr/local/bin/dbx-start-runtime",
    ]
    return "\n".join(script)
