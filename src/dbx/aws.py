...
def build_user_data(config: AppConfig, request: JobLaunchRequest) -> str:
    ...
    script = [
        ...
        f"{codex_command}",
        "CODEX_EXIT=$?",
        "if [ ! -f " + shlex.quote(f"{job_root}/DBX_FINISH_DONE") + " ]; then",
        f"  /usr/local/bin/dbx-finish-job --mode unknown --base {shlex.quote(request.base_branch)} --shutdown",
        "fi",
        "exit \"$CODEX_EXIT\"",
        "DBX_CODEX_WATCH_EOF",
...
def _build_bootstrap_prompt(session_name: str, agent_started_json: str) -> str:
    return "\n".join(
        [
            ...
            "- The instance is disposable; keep the git branch and repository state clean, reviewable, and resumable.",
            ...
            "- before stopping, ensure the repository is clean or clearly document any intentional uncommitted state in STATUS.md",
            "",
            "Start by:",
